    ...
    public static class DiscoveryUtils
    {
        ...
        public static int WebSocketPort(this DiscoveryMessage me)
        {
            ...
            if (!me.Data.TryGetValue(nameof(HostInfo.Port), out var portString))
                throw new WitComException($"Cannot find parameter value for parameter: {nameof(HostInfo.Port)}");
            
            if (!int.TryParse(portString, out int port))
                throw new WitComException($"Wrong value for parameter value for parameter: {nameof(HostInfo.Port)},value: {portString}");
        ...
        public static string WebSocketPath(this DiscoveryMessage me)
        {
            ...
            if (!me.Data.TryGetValue(nameof(HostInfo.Path), out var path))
                throw new WitComException($"Cannot find parameter value for parameter: {nameof(HostInfo.Path)}");

            return path;
        }

        public static WitComClientBuilderOptions WithWebSocket(this WitComClientBuilderOptions me, string host, DiscoveryMessage message, int bufferSize = ClientWebSocketUtils.DEFAULT_BUFFER_SIZE)
        {
