...
{
    [TestFixture]
    public class ProtoSetTests
    {
        [Test]
        public void ConstructorTest()
        {
            var collection = new ProtoSet<int>(1, 2, 3, 4, 5);
        ...
        public void IsTest()
        {
            var collection1 = new ProtoSet<int>(1, 2, 3, 4, 5);
            var collection2 = new ProtoSet<int>(1, 2, 3, 4, 5);

            Assert.That(collection1.Is(collection2), Is.True);
            collection2 = new ProtoSet<int>(2, 2, 3, 4, 5);
            Assert.That(collection1.Is(collection2), Is.False);
            collection1 = new ProtoSet<int>(2, 2, 3, 4, 5);
            Assert.That(collection1.Is(collection2), Is.True);
        }
        ...
        public void MessagePackSerializationTests()
        {
            ...
            var collection1 = bytes.FromProtoBytes<ProtoSet<int>>();
