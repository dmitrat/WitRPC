...
{
    [ProtoContract]
    public class ProtoSet<TValue> : ModelBase, ICollection<TValue>
    {
        #region Fields
        ...
        public ProtoSet(params TValue[] values)
        {
            m_inner = new List<TValue>(values);
        }

        public ProtoSet(IEnumerable<TValue> inner)
        {
        ...
        {
            if (!(modelBase is ProtoSet<TValue> collection))
                return false;
        ...
        {
            return new ProtoSet<TValue>(Inner);
        }
