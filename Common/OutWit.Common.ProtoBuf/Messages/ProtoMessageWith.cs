...
{
    [ProtoContract]
    public class ProtoMessageWith<TData> : ProtoMessage
        where TData :class
    {
        #region Contructors

        private ProtoMessageWith() :
            this("", false)
        {

        }

        public ProtoMessageWith(string message, bool isError) : 
            base(message, isError)
        {
            Data = null;
        }

        public ProtoMessageWith(ProtoMessage message) :
            base(message.Message, message.IsError)
        {
            Data = null;
        }

        public ProtoMessageWith(string message, bool isError, TData? data) :
            base(message, isError)
        {
            Data = data;
        }
        ...
        {
            if (!(modelBase is ProtoMessageWith<TData> message))
                return false;
        ...
        {
            return new ProtoMessageWith<TData>(Message, IsError, CloneData());
        }
